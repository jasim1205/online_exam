...
if (!function_exists('alphabeticSerial')) {
    function alphabeticSerial($index) {
        $alphabet = range('A', 'Z');
        $result = '';
        while ($index > 0) {
            $index--;
...
function employeeId(){
	return encryptor('decrypt', request()->session()->get('empID'));
}

function employeeName(){
	return encryptor('decrypt', request()->session()->get('userName'));
}
